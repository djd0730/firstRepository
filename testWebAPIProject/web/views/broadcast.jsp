...
<body>
	<h1>Boardcast 란?</h1>
	<h3>한 네트워크 속한 모든 사용자가 소통할 수 있는 1:n 의 통신 방식</h3>
	<p>Ex) 라디오 방송, 지상파 TV, 위성 방송</p>
	...
	function connection(){
		webSocket = new WebSocket('ws://192.168.20.96:8088'+'<%=request.getContextPath()%>/broadcast');
	...
	function enterKey(){
		...
		function send(){
			if($inputMessage.val() == ""){
				...
				// 메시지가 입력되었을 경우
				$textarea.html(
						$textarea.html()+"<p class='chat_content'>나 : "+$inputMessage.val().replace(/[\r\n]/gim,"<br>")+"</p><br>");
		...
		function onMessage(event){
			...
			if(content == "" || !sender.match($('#recvUser').val())){
				...
				$textarea.html(
						$textarea.html()+"<p class='chat_content other-side'>"+sender+" : "+content.replace(/[\r\n]/gim,"<br>")+"</p><br>");
