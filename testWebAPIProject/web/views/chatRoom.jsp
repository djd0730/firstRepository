...
<body>
	...
	// 채팅 서버
	var webSocket = new WebSocket('ws://localhost:8088'+'<%=request.getContextPath()%>/multicast/<%=room%>');
	...
	function enterKey(){
		...
		function onMessage(event){
			...
			if(content == ""){
				// 전달 받은 글이 없을 경우 화면에 아무 내용도 표시하지 않겠다.
			}else{
				$textarea.html(
		...
		$('#endBtn').on('click',function(){
			webSocket.send(chat_id+"|님이 채팅방을 종료하였습니다.");
			webSocket.close();
